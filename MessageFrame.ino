...
void loop()
{
  ...
  {
    Sprintln("SCHEDULE: It's time to do a weather update...");
  ...
  {
    Sprintln("SCHEDULE: It's time to do a message check...");
  ...
  {
    Sprintln("SCHEDULE: It's time to see if we should turn the backlight off..");

    int current_hour = hour((now() + tz_gmt_home_offset));
    Sprintln("Current home hour is: " + String(current_hour));
    ...
    if ( (current_hour > 22) && (current_hour < 7)  ) // Turn off blacklight between 10 PM and 7 AM
    {
      if (backlight_on == true)
      {
        backlight_on = false;
        lcd.noBacklight();
        Sprintln("Turning backlight off.");
      }
    }
  ...
  Sprintln("Current displayState: " + String(displayState));
  ...
  switch (displayState)
  {
    ...
    case 4: //  Message
      ...
      Sprintln("Displaying message:");
      Sprintln(current_message);
      ...
      while (pch != NULL) // loop through newlines
      {
        if ( lcd_line++ % 2 == 0 )
        {
          ...
          Sprint("LCD Line 2 (in line buffer): ");
          Sprintln(lcd_line2);
          Sprintln("Str len is: " + String (strlen(lcd_line2)));
        ...
        else
        {
          ...
          lcd.print(lcd_line1);
          Sprint("LCD Line 1 (in line buffer): ");
          Sprintln(lcd_line1);
          Sprintln("Str len is: " + String (strlen(lcd_line1)));
    ...
    default:
      Sprintln("END of valid displayStates.");
      displayState = 1;
...
void getServerTextMessage() {
  ...
  Sprintln(String("GET ") + String(message_url) + " HTTP/1.1\r\n");
  ...
  Sprintln("***************** MESSAGE RESPONSE *****************");
  Sprintln(http_response);
  Sprintln("***************************************************");    
    ...
    {
      Sprintln("There is no message to load from the server!");
    }
    else
    {
      Sprintln("Got server message: " + current_message);
    }
...
void getWeatherCurrentData()
{
  ...
  Sprint("connecting to "); Sprintln(weather_host);
  ...
  if (counter == 0)
  {
    Sprintln("Recieved nothing for some reason.");
  }
...
boolean getTimeFromServer(const char *timezone, boolean is_home_tz)
{
  ...
  Sprintln("Getting time for timezone: " + String(timezone));
  ...
  Sprint(String("GET "    + String(time_url) + "?timezone=" + String(timezone) + " HTTP/1.1\r\n") +
               String("Host: "  + String(our_host) + "\r\n") +
               String("Connection: close\r\n\r\n"));
  ...
  Sprintln("Parsing time server response...");
...
{
  Sprintln("Searching for START KEY: " + key);
  ...
  Sprintln("Start of key is at: " + String(start_pos));
  
  Sprintln("Searching for TERMINATOR KEY:" + terminator);
  int end_pos   = line.indexOf(terminator, start_pos);
  Sprintln("End terminator is at: " + String(end_pos));


  String line2 = line.substring(start_pos + key.length(), end_pos);
  Sprint("The substring is: "); Sprintln(line2);
...
void printLCDbuffers()
{
  ...
  Sprintln("The length of line 1 is: " + String(len));
  if (len <= (LCD_NUM_COLUMNS - 1))
  {
    start_cursor_position = (LCD_NUM_COLUMNS - len) / 2;
  }

  Sprintln("Start pos is: " + String(start_cursor_position));
  ...
  Sprintln("The length of line 2 is: " + String(len));
  if (len <= (LCD_NUM_COLUMNS - 1))
  {
    start_cursor_position = (LCD_NUM_COLUMNS - len) / 2;
  }
